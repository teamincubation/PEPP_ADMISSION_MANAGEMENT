...
// Global Diagnostic Error Catchers (Temporary)
window.onerror = function(message, source, lineno, colno, error) {
    ...
    errDiv.innerHTML = `<strong>JS Error:</strong> ${message}<br>File: ${source}:${lineno}:${colno}`;
    ...
    return originalFetch(...args)
        .then(response => {
            if (!response.ok) {
                ...
                errDiv.style.borderRadius = '8px';
                errDiv.style.zIndex = '99999';
                errDiv.innerHTML = `<strong>Fetch HTTP Error:</strong> Status ${response.status} for URL: ${args[0]}`;
                document.body.appendChild(errDiv);
            }
            return response;
        })
        .catch(err => {
            ...
            errDiv.style.borderRadius = '8px';
            errDiv.style.zIndex = '99999';
            errDiv.innerHTML = `<strong>Fetch Network Error:</strong> ${err.message} for URL: ${args[0]}`;
            document.body.appendChild(errDiv);
            throw err;
        });
};
