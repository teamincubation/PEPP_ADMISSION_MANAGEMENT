...
class PEPPSMTPClient {
    ...
    private function command($cmd, $expectedCode, $isSensitive = false) {
        fwrite($this->socket, $cmd . "\r\n");
        $resp = $this->read();
        $code = (int)substr($resp, 0, 3);
        if ($code !== $expectedCode) {
            $displayCmd = $isSensitive ? '***[SENSITIVE]***' : $cmd;
            throw new Exception("SMTP command failed: '$displayCmd'. Response: '$resp'");
        }
        return $resp;
    }
    ...
    public function send($fromEmail, $fromName, $toEmail, $subject, $bodyHtml, $bodyText = '', array $attachments = []) {
        try {
            ...
            if (!$this->socket) {
                ...
                } elseif (stripos($errstr, 'refused') !== false || $errno === 10061 || $errno === 111) {
            ...
            try {
                ...
                throw new Exception("SMTP server rejected sender");
            }

            try {
                $this->command('RCPT TO:<' . $toEmail . '>', 250);
            } catch (Exception $rcptEx) {
                throw new Exception("Recipient rejected");
            }

            try {
                $this->command('DATA', 354);
            } catch (Exception $dataEx) {
                throw new Exception("SMTP server rejected message");
            }
...
function pepp_mail_dispatch($to, $subject, $bodyHtml, $bodyText = '', array $attachments = [], $fromEmail = '', $fromName = '') {
    ...
    global $pepp_mailer_last_error;
    $pepp_mailer_last_error = '';
    ...
    if ($enabled && $host && $user && $pass) {
        ...
        if ($res) {
            ...
            $pepp_mailer_last_error = $client->getLastError();
            error_log("SMTP dispatch failed, falling back to local mail(): " . $client->getLastError());
        }
    }

    $fallbackRes = pepp_mail_fallback($to, $subject, $bodyHtml, $bodyText, $attachments, $finalFromEmail, $finalFromName);
    if (!$fallbackRes) {
        if (empty($pepp_mailer_last_error)) {
            $pepp_mailer_last_error = "PHP mail() fallback failed";
        } else {
            $pepp_mailer_last_error .= " | PHP mail() fallback failed";
        }
    }
...
function pepp_mailer_get_last_error() {
    global $pepp_mailer_last_error;
    return !empty($pepp_mailer_last_error) ? $pepp_mailer_last_error : 'Provider failed to dispatch message';
}
