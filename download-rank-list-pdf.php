...
class MultiPagePDF {
    ...
    public function output() {
        ...
        foreach ($this->pages as $i => $page) {
            ...
            $res = "/Font << /F1 $helveticaObj 0 R /F2 $helveticaBoldObj 0 R >>";
        ...
        $objs[$helveticaObj] = "<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding >>";
        $objs[$helveticaBoldObj] = "<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold /Encoding /WinAnsiEncoding >>";
...
function draw_page_headers($pdf, $academic_year, $course_name, $chapter_name, $formatted_date) {
    ...
    $pdf->text($lm + 42, $y + 2, 18, 'Mega Test', true);
    $pdf->text($lm + 42, $y + 20, 18, 'Result', true);
    $pdf->setTextColor(0, 0, 0);
    
    // Right side academic details
    $right_x = $rm;
    $w = 200;
    $pdf->text($right_x - $w, $y - 3, 20, $academic_year, true, 'R', $w);
    $pdf->text($right_x - $w, $y + 17, 10, 'Academic Batch', false, 'R', $w);
    $pdf->text($right_x - $w, $y + 28, 11, $course_name, false, 'R', $w);
    ...
    // Chapter Title
    $pdf->text($lm, $y, 11, 'Test: ', true);
    $pdf->setTextColor(234/255, 179/255, 8/255);
    $pdf->text($lm + $pdf->width('Test: ', 11), $y, 11, $chapter_name, true);
    $pdf->setTextColor(0, 0, 0);
    
    $y += 15;
    
    // Test Date
    $pdf->text($lm, $y, 11, 'Test Date: ', true);
    $pdf->setTextColor(234/255, 179/255, 8/255);
    $pdf->text($lm + $pdf->width('Test Date: ', 11), $y, 11, $formatted_date, true);
    $pdf->setTextColor(0, 0, 0);
    ...
    $pdf->text($x_sl, $y, 9.5, 'Sl. No.', true);
    $pdf->text($x_name, $y, 9.5, 'Student Name', true);
    $pdf->text($x_score, $y, 9.5, 'Score', true, 'C', 50);
    $pdf->text($x_rank, $y, 9.5, 'Rank', true, 'C', 50);
...
foreach ($ranking_list as $r) {
    ...
    // Sl. No
    $pdf->text($x_sl, $y_row + 8, 10, $sl_no . '.');
    ...
    // Student Name
    $pdf->text($x_name, $y_row + 8, 10, $r['name']);
    
    // Score (Privacy rule: Hide scores <= 10 with ***)
    ...
    $pdf->text($x_score, $y_row + 8, 10, $score_text, false, 'C', 50);
